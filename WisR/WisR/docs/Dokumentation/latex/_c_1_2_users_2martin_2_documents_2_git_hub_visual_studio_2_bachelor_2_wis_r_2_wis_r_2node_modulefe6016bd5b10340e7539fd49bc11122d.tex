\hypertarget{_c_1_2_users_2martin_2_documents_2_git_hub_visual_studio_2_bachelor_2_wis_r_2_wis_r_2node_modulefe6016bd5b10340e7539fd49bc11122d}{}\section{C\+:/\+Users/martin/\+Documents/\+Git\+Hub\+Visual\+Studio/\+Bachelor/\+Wis\+R/\+Wis\+R/node\+\_\+modules/grunt/node\+\_\+modules/findup-\/sync/node\+\_\+modules/lodash/lodash.\+js}
...
\textcolor{stringliteral}{      // don'}t wrap \textcolor{keywordflow}{if} already wrapped, even \textcolor{keywordflow}{if} wrapped by a different `lodash` constructor
    ...
    var iteratorTemplate = \textcolor{keyword}{template}(
      ...
      \textcolor{stringliteral}{'var index, iterable = <%= firstArg %>, '} +
      \textcolor{comment}{// assign the `result` variable an initial value}
      \textcolor{stringliteral}{'result = <%= init %>;\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'<% if (array) \{ %>\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'  <% if (support.unindexedChars) \{ %>\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'    <%= loop %>;\(\backslash\)n'} +
      \textcolor{stringliteral}{'  \}\(\backslash\)n'} +
      \textcolor{stringliteral}{'\}\(\backslash\)n'} +
      \textcolor{stringliteral}{'else \{'} +
      ...
      \textcolor{stringliteral}{'  <% \} else if (support.nonEnumArgs) \{ %>\(\backslash\)n'} +
      \textcolor{stringliteral}{'  var length = iterable.length; index = -1;\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'  <% if (support.enumPrototypes) \{ %>\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'  <% if (support.enumErrorProps) \{ %>\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'  <% if (useHas && keys) \{ %>\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{"    if (conditions.length) \{ %>    if (<%= conditions.join(' && ') %>) \{\(\backslash\)n  <% \} %>"} +
      \textcolor{stringliteral}{'    <%= loop %>;'} +
      \textcolor{stringliteral}{'    <% if (conditions.length) \{ %>\(\backslash\)n    \}<% \} %>\(\backslash\)n'} +
      \textcolor{stringliteral}{'  \}'} +

      \textcolor{comment}{// else using a for-in loop}
      \textcolor{stringliteral}{'  <% \} else \{ %>\(\backslash\)n'} +
      \textcolor{stringliteral}{'  for (index in iterable) \{\(\backslash\)n<%'} +
      \textcolor{stringliteral}{'    if (useHas) \{ conditions.push("hasOwnProperty.call(iterable, index)"); \}'} +
      \textcolor{stringliteral}{"    if (conditions.length) \{ %>    if (<%= conditions.join(' && ') %>) \{\(\backslash\)n  <% \} %>"} +
      \textcolor{stringliteral}{'    <%= loop %>;'} +
      \textcolor{stringliteral}{'    <% if (conditions.length) \{ %>\(\backslash\)n    \}<% \} %>\(\backslash\)n'} +
      \textcolor{stringliteral}{'  \}'} +

      \textcolor{comment}{// Because IE < 9 can't set the `[[Enumerable]]` attribute of an}
      \textcolor{comment}{// existing property and the `constructor` property of a prototype}
      \textcolor{comment}{// defaults to non-enumerable, Lo-Dash skips the `constructor`}
      \textcolor{comment}{// property when it infers it's iterating over a `prototype` object.}
      \textcolor{stringliteral}{'    <% if (support.nonEnumShadows) \{ %>\(\backslash\)n\(\backslash\)n'} +
      \textcolor{stringliteral}{'  if (iterable !== objectProto) \{\(\backslash\)n'} +
      \textcolor{stringliteral}{"    var ctor = iterable.constructor,\(\backslash\)n"} +
      \textcolor{stringliteral}{'        isProto = iterable === (ctor && ctor.prototype),\(\backslash\)n'} +
      \textcolor{stringliteral}{'        className = iterable === stringProto ? stringClass : iterable === errorProto ? errorClass :
       toString.call(iterable),\(\backslash\)n'} +
      \textcolor{stringliteral}{'        nonEnum = nonEnumProps[className];\(\backslash\)n'} +
      \textcolor{stringliteral}{'      <% for (k = 0; k < 7; k++) \{ %>\(\backslash\)n'} +
      \textcolor{stringliteral}{"    index = '<%= shadowedProps[k] %>';\(\backslash\)n"} +
      \textcolor{stringliteral}{'    if ((!(isProto && nonEnum[index]) && hasOwnProperty.call(iterable, index))<%'} +
      \textcolor{stringliteral}{'        if (!useHas) \{ %> || (!nonEnum[index] && iterable[index] !== objectProto[index])<% \}'} +
      \textcolor{stringliteral}{'      %>) \{\(\backslash\)n'} +
      \textcolor{stringliteral}{'      <%= loop %>;\(\backslash\)n'} +
      \textcolor{stringliteral}{'    \}'} +
      \textcolor{stringliteral}{'      <% \} %>\(\backslash\)n'} +
      ...
      \textcolor{stringliteral}{'    <% \} %>'} +
      \textcolor{stringliteral}{'  <% \} %>'} +
      \textcolor{stringliteral}{'  <% if (array || support.nonEnumArgs) \{ %>\(\backslash\)n\}<% \} %>\(\backslash\)n'} +

      \textcolor{comment}{// add code to the bottom of the iteration function}
      \textcolor{stringliteral}{'<%= bottom %>;\(\backslash\)n'} +
      \textcolor{comment}{// finally, return the `result`}
      \textcolor{stringliteral}{'return result'}
    );
    ...
    \textcolor{keyword}{function} \textcolor{keyword}{template}(text, data, options) \{
      ...
      var imports = iteratorTemplate && defaults(\{\}, options.imports, settings.imports),
          importsKeys = iteratorTemplate ? keys(imports) : [\textcolor{stringliteral}{'\_'}],
    ...
    \textcolor{comment}{// add pseudo private property to be used and removed during the build process}
