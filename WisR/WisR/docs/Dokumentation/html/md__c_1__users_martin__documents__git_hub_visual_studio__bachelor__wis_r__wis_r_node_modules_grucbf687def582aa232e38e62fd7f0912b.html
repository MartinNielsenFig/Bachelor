...
<title>WisR web applikation: Abstract</title>
...
<p>The authorization header and signature would be generated as: </p><pre class="fragment">Authorization: Signature keyId="rsa-key-1",algorithm="rsa-sha256",headers="request-line date content-type content-md5",signature="Base64(RSA-SHA256(signing string))"
</pre><p>The client would compose the signing string as (<code>+ "\\n"</code> inserted for readability): </p><pre class="fragment">POST /foo HTTP/1.1 + "\n"
date: Tue, 07 Jun 2011 20:51:35 GMT + "\n"
content-type: application/json + "\n"
content-md5: h0auK8hnYJKmHTLhKtMTkQ==
</pre><h2>Algorithm</h2>
...
<p>The following test data uses the RSA (2048b) keys, which we will refer to as <code>keyId=Test</code> in the following samples: </p><pre class="fragment">-----BEGIN PUBLIC KEY-----
...
-----END RSA PRIVATE KEY-----
</pre><p>And all examples use this request: </p><pre class="fragment">POST /foo?param=value&amp;pet=dog HTTP/1.1
Host: example.com
...
<p>The string to sign would be: </p><pre class="fragment">date: Thu, 05 Jan 2012 21:31:40 GMT
</pre><p>The Authorization header would be: </p><pre class="fragment">Authorization: Signature keyId="Test",algorithm="rsa-sha256",signature="ATp0r26dbMIxOopqw0OfABDT7CKMIoENumuruOtarj8n/97Q3htHFYpH8yOSQk3Z5zh8UxUym6FYTb5+A0Nz3NRsXJibnYi7brE/4tx5But9kkFGzG+xpUmimN4c3TMN7OFH//+r8hBf7BT9/GmHDUVZT2JzWGLZES2xDOUuMtA="
</pre><h3>All Headers</h3>
<p>Parameterized to include all headers, the string to sign would be (<code>+ "\\n"</code> inserted for readability): </p><pre class="fragment">POST /foo?param=value&amp;pet=dog HTTP/1.1 + "\n"
host: example.com + "\n"
date: Thu, 05 Jan 2012 21:31:40 GMT + "\n"
content-type: application/json + "\n"
content-md5: Sd/dVLAcvNLSq16eXua5uQ== + "\n"
content-length: 18
</pre><p>The Authorization header would be: </p><pre class="fragment">Authorization: Signature keyId="Test",algorithm="rsa-sha256",headers="request-line host date content-type content-md5 content-length",signature="H/AaTDkJvLELy4i1RujnKlS6dm8QWiJvEpn9cKRMi49kKF+mohZ15z1r+mF+XiKS5kOOscyS83olfBtsVhYjPg2Ei3/D9D4Mvb7bFm9IaLJgYTFFuQCghrKQQFPiqJN320emjHxFowpIm1BkstnEU7lktH/XdXVBo8a6Uteiztw="</pre> </div></div><!-- contents -->
<!-- start footer part -->
